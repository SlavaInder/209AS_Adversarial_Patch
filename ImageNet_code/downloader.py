...
parser.add_argument('-scrape_only_flickr', default=False, type=lambda x: (str(x).lower() == 'true'))
parser.add_argument('-number_of_classes', default = 100, type=int)
parser.add_argument('-images_per_class', default = 15, type=int)
parser.add_argument('-data_root', default='content' , type=str)
...
def get_image(img_url):
    ...
    p_indices = [i for i, a in enumerate(get_image.class_folder) if a == "/"]
    last_index = p_indices[-1]
    fold_class_name = get_image.class_folder[last_index+1:]
    
    if int(class_images.value)-1 > 5:
        class_folder = get_image.class_folder[:last_index]+'/testing/'+fold_class_name
    else:
        class_folder = get_image.class_folder[:last_index]+'/validation/'+fold_class_name
    
    list_of_files = glob.glob(class_folder+'/*.jpg')
    try:
        latest_file = max(list_of_files, key=os.path.getctime)
        print(latest_file)
    except:
        if int(class_images.value)-1 > 5:
            img_name = get_image.class_name[:-1]+str(int(class_images.value))+'.jpg'
        else:
            img_name = get_image.class_name+'.jpg'
    else:
        p_indices = [i for i, a in enumerate(latest_file) if a == "/"]
        last_index = p_indices[-1]
        # number = int(latest_file[-5])+1
        number = int(class_images.value)
        
        if (number >10):
            img_name = latest_file[last_index+1:-6]+str(number)+'.jpg'
        else:
            img_name = latest_file[last_index+1:-5]+str(number)+'.jpg'
...
for class_wnid in classes_to_scrape:
    ...
    class_name = class_name.lower().replace(' ', '_')+'s'
    ...
    test_class_folder = os.path.join(imagenet_images_folder, 'testing')
    if not os.path.exists(test_class_folder):
        os.mkdir(test_class_folder)
        
    val_class_folder = os.path.join(imagenet_images_folder, 'validation')
    if not os.path.exists(val_class_folder):
        os.mkdir(val_class_folder)
    
    
    test_class_folder = os.path.join(imagenet_images_folder + '/testing', class_name)
    if not os.path.exists(test_class_folder):
        os.mkdir(test_class_folder)
        
    val_class_folder = os.path.join(imagenet_images_folder + '/validation', class_name)
    if not os.path.exists(val_class_folder):
        os.mkdir(val_class_folder)
        
        
    get_image.class_folder = class_folder

    if hasattr(class_images, "counter"):
        track_values.append(int(class_images.value))
    
    class_images.value = 0

    get_image.class_name = class_name[0:-1]+'0'
    ...
    with Pool(processes=args.multiprocessing_workers) as p:
        ...
        track_names.append(class_name)
        
f = open("content/validation_class_names_indices.txt", "w+")
...
f = open("content/testing_class_names_indices.txt", "w+")

for x in track_names:
    f.write("{} ".format(x))
f.write("\n")
for x in track_names:
    path, dirs, files = next(os.walk("content/imagenet_images/testing/"+x))
    file_count = len(files)
    f.write("7 {} ".format(7+file_count))
f.close()
