...
f = open("content/class_names_indices.txt", "w+")

for x in track_names:
    f.write("{} ".format(x))
f.write("\n")
for x,y in zip(track_names, track_values):
    path, dirs, files = next(os.walk("content/imagenet_images/"+x))
    file_count = len(files)
    f.write("{} {} ".format(0, file_count))
f.close()
